...
export default function Login() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      console.log('requires2FA:', result.requires2FA); // DEBUG
      ...
      // authService.login() already saved the token
  ...
  const handle2FAVerify = async (code, useBackupCode) => {
    ...
    try {
      await authService.loginWith2FA(userEmail, code, useBackupCode);
      
      toast.success("2FA verification successful!");
      
      // authService.loginWith2FA() already saved the token
      // Just navigate to dashboard
      setTimeout(() => {
        window.location.href = '/dashboard';
      }, 500);
